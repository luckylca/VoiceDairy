...
应用同时维护项目和项目需求。本地 Qwen 对话会在每次发送消息前重新读取手机中当前保存的全部项目、项目说明、需求标题和完成状态。当用户明确说明某项需求已经完成时，模型会识别对应需求，应用再把动作映射到真实本地需求并勾选。
...
- 独立本地模型对话页面；
- 云端和本地整理提示词都会附加当前全部项目内容；
- 本地对话自然读取完整项目名、项目说明、需求标题和完成状态；
- 用户明确确认完成后，模型动作和本地高置信度匹配共同定位需求；
- 应用只会把数据库中真实存在且尚未完成的需求设为完成；
...
本地模型管理卡片只在选择“本地 Qwen”时显示；选择“云端 API”时只显示云端 API 配置。

## 项目上下文与自动勾选

项目和需求存储在本地数据库快照中。每次云端或本地智能整理时，应用都会读取当前全部项目，并把完整项目名称、项目说明、需求标题和完成状态附加到系统提示词。
...
模型正常回复用户。只有用户明确说明需求已经完成时，模型才会在回复末尾附加一个不会显示在界面中的短动作标记。应用把短编号映射回数据库中的真实需求 ID，再执行勾选。

为提高小模型可靠性，应用还提供本地安全兜底：

- 用户消息必须包含明确的完成语义；
- 优先匹配完整需求标题；
- 模糊匹配必须达到较高相似度并且与第二候选拉开差距；
- 提到项目名时只在该项目内匹配；
- 不存在、已完成、歧义或低置信度需求不会被修改；
- 最终仍由数据库层校验真实需求 ID。
...
8. 打开本地模型对话，正常询问项目状态或明确说明某项需求已经完成；
9. 在项目详情页检查自动勾选结果。
